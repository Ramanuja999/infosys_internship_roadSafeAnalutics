...
st.markdown("Interact with a massive dataset (500,000 records) to analyze accident trends.")
...
def load_data():
    try:
        ...
        st.error("⚠️ File 'US_Accidents_March23.zip' not found!")
        st.stop()
...
with tab2:
    ...
    if not df_filtered.empty:
        ...
        with col_a:
            ...
            # Clear previous plots to prevent blank screens
            plt.clf() 
            fig1 = plt.figure(figsize=(8, 4))
            sns.histplot(df_filtered['Hour'], bins=24, kde=True, color='teal')
            plt.xlabel("Hour (0-23)")
            plt.xlim(0, 23)
            # Pass the explicit figure object
